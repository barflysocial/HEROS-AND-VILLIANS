...
const els = {
  ...
  playerRoundTitle: document.getElementById('playerRoundTitle'),
  playerRoundStory: document.getElementById('playerRoundStory'),
  playerDangerList: document.getElementById('playerDangerList'),
  playerBroadcastList: document.getElementById('playerBroadcastList'),
  targetDangerSelect: document.getElementById('targetDangerSelect'),
  sidekickDangerSelect: document.getElementById('sidekickDangerSelect'),
    ...
    button.addEventListener('click', async () => {
      ...
      try {
        ...
        await API.post('/api/actions', {
          playerId: player.id,
          power,
          target: els.targetDangerSelect.value || els.targetInput.value
        });
...
function renderPlayerState(state) {
  const { session, broadcasts = [], activeDangers = [], roundInfo = null } = state;
  latestSession = session;
  latestActiveDangers = activeDangers;
  ...
  if (els.playerRoundTitle) els.playerRoundTitle.textContent = roundInfo ? roundInfo.title : 'Power Chest';
  if (els.playerRoundStory) els.playerRoundStory.textContent = roundInfo ? `${roundInfo.story} Objective: ${roundInfo.objective}` : 'Choose the right power for the most urgent danger. Reckless choices can raise destruction and fatalities.';
  els.playerTimer.textContent = formatTimeLeft(session.roundEndsAt);
  renderDangers(els.playerDangerList, activeDangers);
  updateDangerSelect(els.targetDangerSelect, activeDangers, 'Choose danger/task for your power');
  updateDangerSelect(els.sidekickDangerSelect, activeDangers, 'Choose danger/task for sidekick');
...
els.sidekickBtn.addEventListener('click', async () => {
  msg(els.sidekickMsg, '');
  try {
    const task = (els.sidekickDangerSelect.value || els.sidekickTaskInput.value).trim();
